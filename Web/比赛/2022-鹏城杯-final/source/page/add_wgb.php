...
//检查登录状态
   ...
   if (in_array('17',$value)== false) 
   {
	...
	$sql = "select value from config where title='wgbauto'";
	$requ = mysqli_query($con,$sql);
	$rs = mysqli_fetch_array($requ);
	$xxat = $rs['value'];
	$bm = $_SESSION['bm'];
			...
			<div class="layui-inline">
				...
				<div class="layui-input-inline">
					<select name="zclx" lay-verify="required" lay-search="" lay-filter="changeleibie">
						<?php
							$sql = "select id,name from zclx where status=1 and zcfl=3";
							$requ = mysqli_query($con,$sql);
...
<script>
    layui.use(['form', 'laydate', 'upload'], function () {
        ...
        form.on('submit(demo1)', function (data) {
			...
			d=d.replace(/\'/g,"’");
			$.post("../action.php?mode=addwgbgszc",{zz:3,data:d},function(result){
				console.log(result);
