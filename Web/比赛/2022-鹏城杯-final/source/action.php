...
if(!isset($_REQUEST['mode'])){
	...
	if($mode=='getzichanleixing'){//获取资产类型列表
		...
		$l=addslashes($_GET['limit']);
		$p=($p-1)*$l;
		if(isset($_GET['dw'])){
			$dw = ' and zcfl='.addslashes($_GET['dw']);
		}else{
			$dw = '';
		}
	...
	if($mode == 'editjuese'){//编辑角色权限
		...
		$v = addslashes($_POST['value']);
		$u = addslashes($_POST['u']);
		$qxbm = addslashes($_POST['qxbm']);
	...
	if($mode=='searchzichan'){//资产查询
		...
		if(isset($_POST['columns'])){//获取筛选数据
			...
			if(isset($_REQUEST['filterSos'])){//筛选
				$shaixuan = json_decode(urldecode(addslashes($_REQUEST['filterSos'])));
				$xx='';
			...
			if(isset($_GET['rzqi'])){
				$rzqi = addslashes($_GET['rzqi']);
				$rzzhi = addslashes($_GET['rzzhi']);
				$mhss = addslashes($_GET['mhss']);
				if($rzqi == ''){
					$qi = 0;
				}else{
					$qi = strtotime($rzqi);
