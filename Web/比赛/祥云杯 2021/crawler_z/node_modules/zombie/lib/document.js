...
function setupWindow(window, args) {
  ...
  window.WebSocket = function (url, protocol) {
    ...
    const origProperty = (0, _getOwnPropertyDescriptor2.default)(WebSocket.prototype, 'binaryType');
  ...
  window._destroy = function () {
    ...
    for (let i = 0; i < window._length; ++i) if (window[i]) window[i].close();
  ...
  (0, _defineProperties2.default)(windowHistory, {
    length: {
      get() {
  ...
  window._submit = function (formArgs) {
    ...
    const targetWindow = target === '_self' ? window : target === '_parent' ? window.parent : target === '_top' ? window.top : browser.tabs.open({ name: target });
    const modified = (0, _assign2.default)({}, formArgs, { url, target });
...
function getMetaRefresh(document) {
  ...
  if (refresh) {
    ...
    if (match) {
      ...
      if (refreshTimeout >= 0) return { refreshTimeout, refreshURL };
    }
  }
  return null;
}
...
function getHTMLFromResponseBody(buffer, contentType) {
  ...
  if (mimeType) for (let typeOption of typeOptions) if (/^charset=/i.test(typeOption)) {
...
function buildRequest(args) {
  ...
  const params = args.params || new _map2.default();
  ...
  if (/^GET|HEAD|DELETE$/i.test(method)) {
    ...
    for (let [name, values] of params) uri.query[name] = values;
  ...
  if (mimeType === '' || mimeType === 'application/x-www-form-urlencoded') {
    const urlEncoded = [...params].map(function ([name, values]) {
  ...
  if (mimeType === 'multipart/form-data') {
    ...
    for (let [name, values] of params) for (let value of values) form.append(name, value);
...
module.exports = function loadDocument(args) {
  ...
  const document = createDocument((0, _assign2.default)({ url }, args));
  const window = document.defaultView;

  const content = url == 'about:blank' ? '<html><head></head><body></body></html>' : html;
  ...
  window._eventQueue.http(request, (error, response) => {
    if (error) {
      ...
      document.close();
      browser.emit('error', error);
    } else parseResponse({ browser, history, document, response });
  });
  return document;
};
