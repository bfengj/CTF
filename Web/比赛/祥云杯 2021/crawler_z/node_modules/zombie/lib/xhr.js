...
class XMLHttpRequest {
  ...
  constructor(window) {
    ...
    for (let method in EventTarget.interface.prototype) this[method] = EventTarget.interface.prototype[method];
  ...
  abort() {
    ...
    if (this.readyState === XMLHttpRequest.DONE) this.readyState = XMLHttpRequest.UNSENT;else
  ...
  open(method, url, useAsync, user, password) {
    // jshint ignore:line
  ...
  send(data) {
    ...
    this._window._eventQueue.http(request, (error, response) => {
      ...
      if (this._cors) {
        ...
        if (!(allowedOrigin === '*' || allowedOrigin === this._cors)) this._error = new DOMException(DOMException.SECURITY_ERR, 'Cannot make request to different domain');else if (!/^(GET|HEAD|POST)$/.test(this._method)) {
          const allowedMethods = response.headers.get('Access-Control-Allow-Methods');
          if (!allowedMethods || allowedMethods.indexOf(request.method) == -1) this._error = new DOMException(DOMException.SECURITY_ERR, 'Cannot make request with not-allowed method(' + this._method + ')');
        }
        if (this._error) {
          this._browser.errors.push(this._error);
          this._stateChanged(XMLHttpRequest.DONE);
  ...
  _dispatchEvent(event) {
    ...
    if (listener) this[idlUtils.implSymbol]._eventListeners[event.type] = [{
      callback: listener,
      options: {}
    }];
